...
int main(void)
{
    ...
    int ph = 1;
    problem.phases(ph).bounds.lower.states << x1_lower, x2_lower, x3_lower, x4_lower;

    problem.phases(ph).bounds.upper.states << x1_upper, x2_upper, x3_upper, x4_upper;

    problem.phases(ph).bounds.lower.controls << u_lower;
    problem.phases(ph).bounds.upper.controls << u_upper;

    // all events must be zero
    for (int i = 0; i != 8; i++)
    {
        problem.phases(ph).bounds.lower.events(i)=0.0;
        problem.phases(ph).bounds.upper.events(i)=0.0;
    }
    ...
    problem.phases(3).bounds.lower.EndTime      = .0001;
    ...
    auto_phase_guess(problem, control_guess, x_guess, param_guess, time_guess);
