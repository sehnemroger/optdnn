...
int main(void)
{
    ...
    { 
        ...
        problem.phases(j).nodes             = (RowVectorXi(3) <<15,18,50).finished(); // numeros de nós em sequencia
    ...
    problem.phases(1).guess.states.row(0) = linspace(x1_init, x1_init*2/3, nnodes);
    problem.phases(1).guess.states.row(1) = linspace(x2_init, x2_init*2/3, nnodes);
    problem.phases(1).guess.states.row(2) = linspace(x3_init, x3_init*2/3, nnodes);
    problem.phases(1).guess.states.row(3) = linspace(x4_init, x4_init*2/3, nnodes);

    problem.phases(2).guess.states.row(0) = linspace(x1_init*2/3, x1_init*1/3, nnodes);
    problem.phases(2).guess.states.row(1) = linspace(x2_init*2/3, x2_init*1/3, nnodes);
    problem.phases(2).guess.states.row(2) = linspace(x3_init*2/3, x3_init*1/3, nnodes);
    problem.phases(2).guess.states.row(3) = linspace(x4_init*2/3, x4_init*1/3, nnodes);

    problem.phases(3).guess.states.row(0) = linspace(x1_init*1/3, 0, nnodes);
    problem.phases(3).guess.states.row(1) = linspace(x2_init*1/3, 0, nnodes);
    problem.phases(3).guess.states.row(2) = linspace(x3_init*1/3, 0, nnodes);
    problem.phases(3).guess.states.row(3) = linspace(x4_init*1/3, 0, nnodes);

    double tf = 10;

    problem.phases(1).guess.time = linspace(0, tf/3, nnodes);
    problem.phases(2).guess.time = linspace(tf/3, tf*2/3, nnodes);
    problem.phases(3).guess.time = linspace(tf*2/3, tf, nnodes);
